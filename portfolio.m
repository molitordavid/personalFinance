...
methods (Access = private)
  function [obj] = executeTransaction(obj,TA,valueDelta)
    ...
    if TA.isExpense
      realValue = realValue - valueDelta;
    else
      realValue = realValue + valueDelta;
    end
    AC = AC.edit(AC.institution,AC.accountId,AC.unrealValue,realValue,AC.date,AC.name,AC.type);
  ...
  function [obj] = executeTransfer(obj,TR,valueDelta)
    ...
    toAC = toAC.edit(toAC.institution,toAC.accountId,toAC.unrealValue,newToAccRealValue,toAC.date,toAC.name,toAC.type);
    fromAC = fromAC.edit(fromAC.institution,fromAC.accountId,fromAC.unrealValue,newFromAccRealValue,fromAC.date,fromAC.name,fromAC.type);
